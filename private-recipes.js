...
.accountBtn{display:inline-flex;align-items:center;gap:7px;border:1px solid rgba(241,200,91,.28);background:rgba(255,255,255,.04);color:var(--text);border-radius:999px;padding:9px 13px;cursor:pointer;font-weight:700}.accountBtn:hover{border-color:rgba(241,200,91,.55)}
...
function updateAccount(){const b=$("#accountBtn");if(!b)return;b.innerHTML=S.user?'<b>♙</b><span>Cuenta</span>':'<b>♙</b><span>Iniciar sesión</span>'}
...
function renderList(){const l=$("#myList");if(!l)return;if(!S.recipes.length){l.innerHTML='<div class="privateEmpty">Todavía no tienes recetas guardadas.<br><br><button class="chip primary" id="firstAdd">＋ Agregar mi primera receta</button></div>';$("#firstAdd").onclick=form;return}l.innerHTML=S.recipes.map(r=>'<div class="privateCard"><div>'+(img(r)?'<img src="'+img(r)+'" alt="'+esc(r.name)+'">':'<div class="privateThumb" style="display:grid;place-items:center;color:var(--gold);font-size:24px">✦</div>')+'</div><div><h3>'+esc(r.name)+'</h3><p>'+esc(r.category||"Mi receta")+' · '+esc(r.ingredients)+'</p></div><div class="privateCardActions"><button class="chip" data-edit="'+r.id+'">Editar</button><button class="chip" data-del="'+r.id+'">Eliminar</button></div></div>').join("");l.querySelectorAll("[data-edit]").forEach(b=>b.onclick=()=>form(S.recipes.find(r=>r.id===b.dataset.edit)));l.querySelectorAll("[data-del]").forEach(b=>b.onclick=()=>del(b.dataset.del))}
...
function init(){
 style();const fav=$("#favFilter");if(!fav)return;const b=document.createElement("button");b.id="accountBtn";b.className="accountBtn";b.type="button";fav.insertAdjacentElement("afterend",b);b.onclick=()=>ensure().then(account).catch(()=>auth("login"));updateAccount();
 const ao=overlay("authOverlay","Cuenta"),co=overlay("accountOverlay","Mi cuenta"),mo=overlay("myOverlay","Mis recetas"),fo=overlay("formOverlay","Agregar receta");
