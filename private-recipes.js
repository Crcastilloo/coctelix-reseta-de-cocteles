...
function auth(mode="login"){
 ...
 b.innerHTML='<div class="privateTabs"><button class="chip '+(mode==="login"?"active":"")+'" data-mode="login">Iniciar sesión</button><button class="chip '+(mode==="signup"?"active":"")+'" data-mode="signup">Crear cuenta</button></div><form class="privateForm" id="authForm"><div class="privateField"><label>Correo electrónico</label><input id="aEmail" type="email" autocomplete="email" required></div><div class="privateField"><label>Contraseña</label><input id="aPass" type="password" minlength="8" required></div>'+(mode==="login"?'<button id="forgotBtn" class="authLink" type="button">¿Olvidaste tu contraseña?</button>':"")+'<div id="aMsg" class="privateMsg"></div><button id="aSubmit" class="chip primary" type="submit">'+(mode==="login"?"Iniciar sesión":"Crear cuenta")+'</button></form>';
 b.querySelectorAll("[data-mode]").forEach(x=>x.onclick=()=>auth(x.dataset.mode));$("#authForm").onsubmit=authSubmit;if(mode==="login")$("#forgotBtn").onclick=forgotPassword;open(o)
}
async function forgotPassword(){
 const email=$("#aEmail").value.trim(),msg=$("#aMsg"),btn=$("#forgotBtn");
 if(!email){msg.textContent="Escribe primero tu correo electrónico.";msg.className="privateMsg error";$("#aEmail").focus();return}
 btn.disabled=true;btn.textContent="Enviando…";msg.textContent="";msg.className="privateMsg";
 try{const c=client();if(!c)throw Error("El servicio de cuenta no está disponible.");const {error}=await c.auth.resetPasswordForEmail(email,{redirectTo:location.origin+location.pathname});if(error)throw error;msg.textContent="Te enviamos un correo para restablecer tu contraseña. Revisa también Spam o Promociones.";toast("Correo de recuperación enviado")}catch(x){msg.textContent=errText(x);msg.className="privateMsg error"}finally{btn.disabled=false;btn.textContent="¿Olvidaste tu contraseña?"}
}
function resetForm(){
 const o=$("#authOverlay"),b=o.querySelector(".privateBody");
 b.innerHTML='<div class="resetNotice">Crea una nueva contraseña para tu cuenta de Cóctelix.</div><form class="privateForm" id="resetForm"><div class="privateField"><label>Nueva contraseña</label><input id="rPass1" type="password" minlength="8" autocomplete="new-password" required></div><div class="privateField"><label>Repetir contraseña</label><input id="rPass2" type="password" minlength="8" autocomplete="new-password" required></div><div id="rPassMsg" class="privateMsg"></div><div class="privateActions"><button id="rPassSubmit" class="chip primary" type="submit">Cambiar contraseña</button><button id="rPassCancel" class="chip" type="button">Cancelar</button></div></form>';
 $("#resetForm").onsubmit=async e=>{
  e.preventDefault();const p1=$("#rPass1").value,p2=$("#rPass2").value,msg=$("#rPassMsg"),btn=$("#rPassSubmit");
  msg.textContent="";msg.className="privateMsg";
  if(!/^(?=.*[A-Za-z])(?=.*\d).{8,}$/.test(p1)){msg.textContent="La contraseña debe tener al menos 8 caracteres e incluir letras y números.";msg.className="privateMsg error";return}
  if(p1!==p2){msg.textContent="Las contraseñas no coinciden.";msg.className="privateMsg error";return}
  btn.disabled=true;btn.textContent="Guardando…";
  try{const c=client();if(!c)throw Error("El servicio de cuenta no está disponible.");const {error}=await c.auth.updateUser({password:p1});if(error)throw error;close(o);toast("Contraseña actualizada correctamente")}catch(x){msg.textContent=errText(x);msg.className="privateMsg error"}finally{btn.disabled=false;btn.textContent="Cambiar contraseña"}
 };
 $("#rPassCancel").onclick=()=>close(o);open(o)
}
...
function splitPrivateIngredients(s){s=s.replace(/\s+/g," ").trim();if(!s)return[];const starts=[];const re=/(\d+(?:-\d+)?(?:\s+\d+\/\d+|\/\d+)?)\s*(?:oz|cups?|tsp|tbsp|teaspoons?|tablespoons?|bar spoon|shots?|slices?|sprigs?|leaves?|berries|cherries)\b/gi;let m;while((m=re.exec(s))!==null)starts.push(m.index);const rim=s.search(/\bFor the Rim:/i);if(rim>0)starts.push(rim);starts.sort((a,b)=>a-b);const out=[];for(let i=0;i<starts.length;i++){const a=starts[i],b=i+1<starts.length?starts[i+1]:s.length;const x=s.slice(a,b).trim();if(x)out.push(x)}return out.length?out:[s]}
function openPrivateRecipe(r){if(!r)return;const photo=img(r);$("#mimage").src=photo||"";$("#mimage").alt=r.name;$("#mnum").textContent="Mi receta";$("#mname").textContent=r.name;const parts=splitPrivateIngredients(r.ingredients||"No especificado.");$("#ming").innerHTML=parts.map(x=>`<div class="ingredient">${esc(x)}</div>`).join("")||`<div class="ingredient">No especificado</div>`;$("#gsection").style.display=r.garnish?"block":"none";$("#mgarnish").textContent=r.garnish||"";$("#minst").textContent=r.preparation||"No especificada.";$("#mfav").style.display="none";$("#copy").textContent="Copiar receta";$("#copy").onclick=()=>copyPrivateRecipe(r);$("#overlay").classList.add("show")}
